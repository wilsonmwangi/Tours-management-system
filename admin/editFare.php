<?php 
	...
	if(isset($_POST['editFare'])){
		$fare=$obj->getFareDetails($conn, $_GET['fid']);
		 ...
		 if(!$pickfromError && !$droptoError && !$fleetError && !$fareError){
			$obj->updateFare($conn, $pickfrom, $dropto, $fleet, $fare, $_GET['fid']);
	...
	<div class="panel-body">
		<form class="col-md-8 col-xs-12" method="POST">
			...
			<div class="form-group">
				...
				<input type="text" class="form-control pickDrop" name="pickfrom" placeholder="Enter a pickup location" value="<?php echo $fare['pickfrom']; ?>" />
				<?php if($pickfromError) echo'<p class="text-danger"><i class="fa fa-times-circle animated flash"></i> '.$pickfromError.'</p>'; ?>
			</div>
			<div class="form-group">
				<label>Drop To</label>
				<input type="text" class="form-control pickDrop" name="dropto" placeholder="Enter a drop to location" value="<?php echo $fare['dropto']; ?>" />
			...
			<div class="form-group">
				...
				<div class="btn-group fleetsList">
					<input type="hidden" name="fleet" id="fleet" value="<?php echo $fare['fleet']; ?>" />
					<?php if($_POST['fleet']){ ?>
					...
					<?php } else { ?>
					<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo $fare['fleet']; ?></button>
					<?php } ?>
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" name="fare" placeholder="Enter fare" value="<?php echo $fare['fare']; ?>" />
				<?php if($fareError) echo'<p class="text-danger"><i class="fa fa-times-circle animated flash"></i> '.$fareError.'</p>'; ?>
			</div>
			<hr>
			<div class="form-group">
				<button type="submit" name="editFare" class="btn btn-success">Update</button>
			</div>
		</form>
	</div>
</div>
